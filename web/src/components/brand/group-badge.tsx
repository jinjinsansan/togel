 ...
 *
 * そのため GROUP_BADGE は export しない。外に出すのは「必ず両方を描画する」
 * 3つの入口（GroupBadge / GroupBadgeOg / groupBadgeLine）と、名前を含まない groupEmoji だけ。
 */
  ...
  if (variant === "light") {
    return (
      <div
        ...
        aria-label={groupBadgeLine(group)}
      >
        <span className="text-[12px] font-black text-lighttext">
          {emoji} {label}
        </span>
        <span className="text-[10px] text-lighttext-subtle">{redefinition}</span>
      </div>
    );
  }
  ...
  if (variant === "section") {
    return (
      <div className={`flex flex-col gap-0.5 ${className}`} aria-label={groupBadgeLine(group)}>
        ...
        <span className="text-[12px] text-txt-muted">{redefinition}</span>
      </div>
    );
  }
